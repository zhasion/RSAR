...
class RSARDataset(CustomDataset):
    ...
    PALETTE = [(220, 20, 60), (0, 0, 230), (106, 0, 228),
    ...
    def __init__(self,
        ...
        super(RSARDataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        ...
        if not ann_files:  # test phase
            ...
            for ann_file in ann_files:
                ...
                img_id = osp.split(ann_file)[1][:-4]
                img_name = img_id + '.png'
                if not osp.exists(osp.join(ann_folder.replace("annfiles", "images"), img_name)):
                    img_name = img_id + '.jpg'
                    if not osp.exists(osp.join(ann_folder.replace("annfiles", "images"), img_name)):
                        img_name = img_id + '.bmp'
                data_info['filename'] = img_name
