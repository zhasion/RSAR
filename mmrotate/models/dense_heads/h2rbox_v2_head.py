...
class H2RBoxV2Head(RotatedFCOSHead):
    ...
    def loss_by_feat(
        ...
        if hasattr(self.angle_coder, 'loss_angle_restrict') and self.angle_coder.loss_angle_restrict:
            if len(pos_inds) > 0:
                losses.update(loss_angle_restrict = self.angle_coder.get_restrict_loss(pos_angle_preds))
        return losses
